...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
        ...
        if (!preg_match('/^\d{8}$/', $idno)) {
            header("Location: ../auth/register.php?error=invalid_id");
            exit();
        }
        ...
        $stmt->execute([$idno]);
        if ($stmt->rowCount() > 0) {
            header("Location: ../auth/register.php?error=id_exists");
            exit();
        }
        ...
        if ($stmt->rowCount() > 0) {
            header("Location: ../auth/register.php?error=username_exists");
            exit();
        }
        ...
        header("Location: ../auth/register.php?error=database");
        exit();
    }
}
?>
